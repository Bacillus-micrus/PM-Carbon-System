...
postNN_OldPT <- mutate(postNN_OldPT, source = "Old PT")
postNN_ICES <- read_csv("postNN_CO2sys_ICESfiltered.csv", na = "NaN")
postNN_ICES <- mutate(postNN_ICES, source = "ICES")
...
onlypH_OldPT <- postNN_OldPT %>%  # A new data.frame from the loaded csv (and we keep it intact as postNN*)
  ...
  select(G2year, pH, source) %>%  # Clean it all
...
onlypH_G2 <- A %>%
  ...
  select(G2year, pH, country, source)
...
ggplot(finalA, aes(G2year, pH))+
  geom_jitter(data = extendedA, aes(colour = source), alpha = .7, shape = 4, size = .3) +
...
finalXcCO3 <- onlyXcCO3 %>%
  group_by(G2year) %>%
  summarise_at(vars(xcCO3), mean, na.rm = TRUE)

finalXcCO3Error <- onlyXcCO3 %>%
  group_by(G2year) %>%
...
RLMmodelXcCo3 <- MASS::rlm(xcCO3 ~ G2year, data = finalXcCO3, weights = (1 / finalXcCO3Error$xcCO3))

tableRLMmodelXcCo3 <- summary(RLMmodelXcCo3)
trendXcCo3 <- tableRLMmodelXcCo3[["coefficients"]][2, 1]  # This is the rate of change
etrendXcCo3 <- tableRLMmodelXcCo3[["coefficients"]][2, 2]  # etrend stands for "error in the trend"

pvalueCant <- pnorm(
  abs(
    broom::tidy(RLMmodelXcCo3)
    [2, "statistic", drop = TRUE]), lower.tail = FALSE) * 2
...
ggplot(finalXcCO3, aes(G2year, xcCO3)) +
  geom_smooth(method = MASS::rlm, color = "red", size = 2) +
  geom_point() +
  theme_minimal()
