...
```{r Carbon error computations}

carb_errors <- seacarb::errors(flag = A$flag, 
                          var1 = ifelse(A$flag == 15, A$G2talk / 10^6, 
                                      ifelse(A$flag == 9, A$G2phtsinsitutp,
                                             ifelse(A$flag == 8, A$G2phtsinsitutp, NA))),
                          var2 = ifelse(A$flag == 15, A$G2tco2 / 10^6, 
...
# postNN_OldPT$G2year <- as.factor(postNN_OldPT$G2year)
# postNN_OldPT$pH <- as.integer(postNN_OldPT$pH)
...
# postNN_ICES$G2year <- as.factor(postNN_ICES$G2year)
# postNN_ICES$pH <- as.integer(postNN_ICES$pH)
```

```{r Checking for duplicate data (GLODAP & ICES)}
...
#   select(G2year, pH, source, country, zone)  # Clean it all
#   
...
#   select(G2year, pH, source, country, zone)
# 
...
# 
# 
# list <- list(pH, pH_NN, only_pH_G2)  # my idea here is to create a custom function and apply it with lapply to all the data combinations automatically
# 
...
(basemap <- ggplot(m, aes(x = x, y = y)) +  # when just working with shapes, the map gets really confusing... Should I add colours on top of that or is there a better solution. 22 stations are quite a bit
